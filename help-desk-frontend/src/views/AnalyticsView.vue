...
watch([selectedYear, selectedMonth], ([newYear, newMonth]) => {
  ticketStore.setDateFilter(newMonth, newYear)
})
...
async function printReport(reportId: string, reportName: string, reportTitle: string) {
  ...
  header.innerHTML = `
    <img src="${logoBrisa}" style="width: 80px; margin-bottom: 10px;" />
    <h2>${reportTitle}</h2>
    ...
    <header class="view-header">
      ...
      <div class="filters">
        ...
        <select v-model="selectedMonth">
          ...
          <option
            v-for="month in ticketStore.availableMonths"
            :key="month.value"
            :value="month.value"
          >
            {{ month.text }}
    ...
    <div class="tab-content">
      <div v-if="activeTab === 'byAnalyst'">
        ...
        <div class="action-buttons">
          ...
          <button
            @click="
              exportReportAsCsv(ticketStore.analystPerformanceReport, 'dashboard_por_analista', [
                'Analista',
                'Chamados Atendidos',
              ])
            "
      ...
      <div v-if="activeTab === 'byCategory'">
        ...
        <div class="action-buttons">
          ...
          <button
            @click="
              exportReportAsCsv(
                ticketStore.avgResolutionTimeByCategoryReport,
      ...
      <div v-if="activeTab === 'byMonth'">
        ...
        <div class="action-buttons">
          ...
          <button
            @click="
              exportReportAsCsv(ticketStore.ticketsPerMonthReport, 'dashboard_por_mes', [
                'Mês/Ano',
                'Total de Chamados',
              ])
            "
